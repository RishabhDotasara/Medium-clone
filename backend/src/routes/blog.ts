...
blogRouter.get("/", authMiddleware, async (c) => {
  ...
  try {
    ...
    //get the users following list so that we can mark blogs as following or not.
    ...
    const followingList = await user?.following;
    ...
    await blogs.map((blog) => {
      ...
      blogsToSend.push({
        ...blog,
        list:followingList,
        bookmarked: bookmarks.find((bookmark) => {
          return bookmark.postID == blog.id;
        })
          ? true
          : false,
        following: followingList?.includes(blog.authorId),
