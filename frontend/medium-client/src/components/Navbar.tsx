      ...
      <div className='flex gap-9 items-center justify-center'>
        ...
        {notActive && <div className='notifications-list absolute bg-white top-20 right-0 shadow-lg p-3 rounded w-64'>
          {notifications.length != 0  && notifications.map((notification:any)=>{
            return (
              <Link to={"/blog/"+notification.blogLink} ><Notification notification={notification} remove={removeNotification}/></Link>
            )
          })}
          {notifications.length ==0 && (<h1 className='w-full text-center font-semibold'>No new notifications!</h1>)}
        </div>}
        </div>
        <div className='rounded-full h-10 w-10 flex items-center justify-center  cursor-pointer' onClick={()=>{setActive(!active)}}><FaRegUser className='text-2xl'/></div>
